...
public final class WorldLoadingProgress {
    ...
    public static void finish(String reason) {
        if (!active && !suspendedForBlockingScreen) {
            suppressVanillaLoadingRenderer = false;
            waitingForFirstWorldRender = false;
            waitingForFirstWorldRenderAt = 0L;
            return;
    ...
    public static boolean isDisconnectScreen(GuiScreen screen) {
        if (screen == null) {
            return false;
        }
        ...
        String className = screen.getClass().getName();
        return className.endsWith(".GuiDisconnected")
                || className.endsWith(".DisconnectedScreen")
                || className.contains("RealmsDisconnected");
    }
