...
public final class LibVulpesCompatibilityTransformer implements IClassTransformer {
    private static final boolean ENABLED = Boolean.parseBoolean(System.getProperty(
            "gpom.libvulpes.lazyProductBlockLists",
            "true"
    ));
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        ...
        if ((className != null && className.startsWith("com.luna.gpom."))
                || !"zmaster587.libVulpes.api.material.MaterialRegistry".equals(className)
                || !TargetedModVersions.isLibVulpesClass(className)) {
            return basicClass;
        }
