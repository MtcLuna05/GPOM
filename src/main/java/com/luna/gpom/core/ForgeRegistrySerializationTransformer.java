...
public final class ForgeRegistrySerializationTransformer implements IClassTransformer {
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        ...
        if (className != null && className.contains("com.luna.gpom.")) {
            return basicClass;
        }
        ...
        for (AbstractInsnNode insn = method.instructions.getFirst(); insn != null; insn = insn.getNext()) {
            if (insn.getOpcode() == Opcodes.ARETURN) {
                ...
                snapshot.add(new MethodInsnNode(
                        ...
                        "com/luna/gpom/optimization/ForgeRegistrySnapshotOptimizations",
                        helperMethod,
                        helperDesc,
                        false
                ));
    ...
    private static boolean patchGameDataInjectSnapshotMigration(MethodNode method) {
        ...
        injected.add(new MethodInsnNode(
                Opcodes.INVOKESTATIC,
                "com/luna/gpom/optimization/BlockcrafteryRegistryMigration",
                "normalizeSnapshot",
                "(Ljava/util/Map;)V",
                false
        ));
    ...
    private static boolean patchGameDataRegisterImplWorkerQueue(MethodNode method) {
        ...
        for (AbstractInsnNode insn = method.instructions.getFirst(); insn != null; insn = insn.getNext()) {
            if (insn.getOpcode() == Opcodes.INVOKESTATIC
                    ...
                    && "com/luna/gpom/optimization/RegistryEventParallelDispatcher".equals(((MethodInsnNode) insn).owner)
        ...
        injected.add(new MethodInsnNode(
                Opcodes.INVOKESTATIC,
                "com/luna/gpom/optimization/RegistryEventParallelDispatcher",
    ...
    private static boolean patchIngredientClinit(String classInternalName, MethodNode method) {
        ...
        for (AbstractInsnNode insn = method.instructions.getFirst(); insn != null; insn = insn.getNext()) {
            ...
            wrap.add(new MethodInsnNode(
                    Opcodes.INVOKESTATIC,
                    "com/luna/gpom/optimization/IngredientConcurrencyGuards",
    ...
    private static boolean hasHelperCall(MethodNode method, String helperMethod) {
        for (AbstractInsnNode insn = method.instructions.getFirst(); insn != null; insn = insn.getNext()) {
            if (insn.getOpcode() == Opcodes.INVOKESTATIC
                    && insn instanceof MethodInsnNode
                    && "com/luna/gpom/optimization/IngredientConcurrencyGuards".equals(((MethodInsnNode) insn).owner)
