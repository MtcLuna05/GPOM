package com.luna.gpom.compat.agricraft;
...
public final class AgriCraftChannelConnectionCompat {
    ...
    private static final Map<Block, Boolean> WATER_CHANNEL_BLOCKS = new ConcurrentHashMap<>();
    ...
    private static boolean isAgricraftWaterChannel(IBlockState state) {
        ...
        Boolean cached = WATER_CHANNEL_BLOCKS.get(block);
        if (cached != null) {
            return cached;
        }
        Object registryName = MinecraftMappingCompat.blockRegistryName(block);
        String text = registryName == null ? null : registryName.toString();
        boolean channel = text != null && text.startsWith("agricraft:water_channel_");
        Boolean existing = WATER_CHANNEL_BLOCKS.putIfAbsent(block, channel);
        return existing != null ? existing : channel;
    }
