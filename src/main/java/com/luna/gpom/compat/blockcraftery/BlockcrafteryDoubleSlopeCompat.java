package com.luna.gpom.compat.blockcraftery;

import com.luna.gpom.GPOM;
import com.luna.gpom.compat.framed.FramedMaterialData;
import com.luna.gpom.compat.minecraft.MinecraftMappingCompat;
...
public final class BlockcrafteryDoubleSlopeCompat {
    ...
    public static void read(BlockcrafteryDoubleSlopeAccess access, NBTTagCompound root) {
        ...
        NBTTagCompound previous = access.gpom$getDoubleSlopeData();
        boolean renderPayloadChanged = false;
        if (root == null || !MinecraftMappingCompat.nbtHasKey(root, TAG)) {
            if (previous != null) {
                access.gpom$setDoubleSlopeData(null);
                renderPayloadChanged = true;
            }
        } else {
            ...
            if (MinecraftMappingCompat.nbtIsEmpty(data)
                ...
                access.gpom$setDoubleSlopeData(null);
                renderPayloadChanged = previous != null;
            } else {
        ...
        if (renderPayloadChanged && access instanceof TileEntity) {
            ...
            if (world != null && pos != null && MinecraftMappingCompat.worldIsRemote(world)) {
                IBlockState state = MinecraftMappingCompat.worldBlockState(world, pos);
                if (state != null) {
                    // Blockcraftery's native material mutation uses this exact
                    // client notification to invalidate its terrain model.
                    MinecraftMappingCompat.worldNotifyBlockUpdate(world, pos, state, state, 8);
                }
            }
        }
    }
    ...
    public static IBlockState attachSecondary(IBlockState state, IBlockAccess world, BlockPos pos) {
        ...
        if (!extendedState.getUnlistedNames().contains(BlockcrafteryDoubleSlopeStateProperty.INSTANCE)) {
            return state;
        }
        ...
        try {
            return extendedState.withProperty(
    ...
    public static IBlockState secondaryState(IExtendedBlockState state) {
        if (state == null
                || !state.getUnlistedNames().contains(BlockcrafteryDoubleSlopeStateProperty.INSTANCE)) {
            return null;
        }
        try {
    ...
    private static void sync(TileEntity tile, World world, BlockPos pos, IBlockState hostState) {
        ...
        // Bit 2 carries Blockcraftery's full tile update packet. The receiving
        // client's read hook performs the terrain invalidation after installing
        // the new secondary payload; the block state itself does not change.
