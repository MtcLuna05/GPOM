package com.l.cleanroomoptimizations.util;
...
public final class ReflectionFields {
    ...
    private static void logOnce(Class<?> ownerClass, String purpose, String action, Exception e) {
        ...
        if (e == null) {
            CleanroomOptimizations.LOGGER.warn("Skipping cleanup for {}.{}: {} field", ownerClass.getName(), purpose, action);
        } else {
            CleanroomOptimizations.LOGGER.warn("Skipping cleanup for {}.{}: {} field", ownerClass.getName(), purpose, action, e);
        }
    }
}
