...
public final class StartupProfiler {
    private static final boolean ENABLED = Boolean.parseBoolean(System.getProperty("cleanroomoptimizations.startupProfiler", "true"));
    ...
    public static void beginPhase(String phaseName) {
        ...
        synchronized (LOCK) {
            ...
            CleanroomOptimizations.LOGGER.info("[StartupProfiler] Starting FML phase {}", phaseName);
        }
    }
    ...
    public static void recordModError(ModContainer container, FMLEvent event, long startedAt, Throwable throwable) {
        ...
        if (ENABLED && container != null && event != null) {
            CleanroomOptimizations.LOGGER.warn(
    ...
    private static String progressTitle(String probeName) {
        if (probeName.startsWith("THAUM ")) {
            return "CRO Thaumcraft";
        }
        if (probeName.startsWith("HEI ")) {
            return "CRO HEI";
        }
        if (probeName.startsWith("MM ")) {
            return "CRO Modular Machinery";
        }
        return "CleanroomOptimizations";
    }
    ...
    private static String shortProgressName(String probeName) {
        ...
        String prefix = probeName.startsWith("thaumcraft.") ? "CRO Thaum: " : probeName.startsWith("hellfirepvp.modularmachinery.") ? "CRO MM: " : "CRO HEI: ";
    ...
    public static final class StackSampler {
        ...
        private void start() {
            Thread samplerThread = new Thread(this::run, "CleanroomOptimizations Startup Stack Sampler - " + modId);
        ...
        private void logStackSample() {
            ...
            for (int i = 0; i < limit; i++) {
                CleanroomOptimizations.LOGGER.info("[StartupProfiler] [StackSample]   at {}", stackTrace[i]);
            }
        }
