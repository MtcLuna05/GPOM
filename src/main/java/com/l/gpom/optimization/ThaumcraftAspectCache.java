...
public final class ThaumcraftAspectCache {
    ...
    public static void beginPostInit() {
        ...
        if (depth == 0) {
            ...
            if (ENABLED) {
                ...
                GPOM.LOGGER.info("[Thaumcraft Optimizations] Enabled generated aspect cache for Thaumcraft PostInit");
            }
        }
    ...
    public static void endPostInit() {
        ...
        if (depth == 0) {
            GPOM.LOGGER.info("[Thaumcraft Optimizations] ThaumcraftApi.exists replacement during PostInit: hits={}, misses={}", EXISTS_HITS.sum(), EXISTS_MISSES.sum());
            if (ENABLED) {
                GPOM.LOGGER.info("[Thaumcraft Optimizations] Cleared generated aspect cache after Thaumcraft PostInit (entries={})", GENERATED_TAGS.size());
                GENERATED_TAGS.clear();
            }
        }
    }
